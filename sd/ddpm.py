...
class DDPMSampler: 
    ...
    def set_inference_timesteps(self, num_inference_steps=50):
        ...
        stepratio = self.num_training_steps // num_inference_steps
        timesteps = (np.arange(0, num_inference_steps) * stepratio )[::-1].round().astype(np.int64).copy()
    ...
    def add_noise(self, original_samples: torch.FloatTensor, timesteps: torch.IntTensor):
        ...
        # From equation (4) of the DDPM paper
        noise = torch.randn(original_samples.shape, generator=self.generator, device=original_samples.device, dtype=original_samples.dtype)
        
        noise_mean = sqrt_alpha_cumprod * original_samples
        noise_stdev = sqrt_one_minus_alpha_cumprod
        noisy_samples = noise_mean + noise_stdev * noise
